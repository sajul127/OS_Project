#include "PolicyNUR.h"
#include <algorithm>
#include <cctype> // std::toupper, std::islower 사용
...
void PolicyNUR::Operate(char data) {
    ...
    // 대문자는 '읽기', 소문자는 '쓰기'로 간주하고 실제 메모리에는 대문자로 적재
    ...
    if (it != frames.end()) {
        ...
        // Hit 발생 시 R 비트는 무조건 1, 쓰기 작업이었다면 M 비트도 1로 갱신
        ...
        if (currentCount < frameSize) {
            ...
            refBits[currentCount] = true;         // 새로 적재 시 R=1
        ...
        // 3. Migration (빈 공간이 없어 교체해야 하는 경우)
            ...
            int victimRow = -1;
            int minClass = 4; // 클래스는 0~3까지 있으므로 4로 초기화

            // clockHand부터 시작하여 한 바퀴를 돌며 가장 낮은 클래스(0~3)를 찾음
            for (int i = 0; i < frameSize; ++i) {
                ...
                if (currentClass < minClass) {
                    ...
                    victimRow = idx;
                    // 가장 최하위 클래스인 0을 찾았다면 더 찾을 필요 없이 즉시 교체
                    if (minClass == 0) break;
                }
            }
            ...
            // 다음 교체 시 공평한 탐색을 위해 바늘을 희생자 다음 칸으로 이동
    ...
    // 전체 요청(Hit + Fault)이 5번 발생할 때마다 모든 R비트를 0으로 리셋!
