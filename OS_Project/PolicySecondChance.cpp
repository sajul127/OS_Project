...
void PolicySecondChance::Operate(char data) {
    ...
    if (it != frames.end()) {
        ...
        // Hit 발생 시 참조 비트를 1(true)로 갱신 (핵심 로직)
        ...
        if (currentCount < frameSize) {
            ...
            refBits[currentCount] = true; // 새로 메모리에 적재될 때 참조 비트는 1
        ...
        // 3. Migration (빈 공간이 없어 쫓아내야 하는 경우)
            ...
            while (true) {
                if (refBits[clockHand] == true) {
                    // 기회를 한 번 주고(0으로 변경), 바늘을 다음으로 이동
                    refBits[clockHand] = false;
                    clockHand = (clockHand + 1) % frameSize;
                }
                else {
                    // 참조 비트가 0인 프레임을 발견하면 교체 (희생자 당첨)
                    frames[clockHand] = data;
                    refBits[clockHand] = true; // 새로 들어왔으므로 1로 설정
                    record.targetRow = clockHand;

                    // 다음 교체를 위해 바늘을 한 칸 전진시켜둠
                    clockHand = (clockHand + 1) % frameSize;
                    break;
                }
            }
    ...
    record.refBitSnap = refBits; // (Step 1을 적용했다면 주석 해제)
    record.clockHand = clockHand; // (Step 1을 적용했다면 주석 해제)
